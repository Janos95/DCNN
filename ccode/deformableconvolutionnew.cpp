...
static double bilinearInterp(THDoubleTensor* input, THLongTensor* bi, THDoubleTensor* bw, long c, double py, double px, std::size_t i, std::size_t j, long save_buffer){
    ...
    if(px_projected != px or py_projected != py){
        w[0] *= (1-std::abs(py-py_projected));
        w[1] *= (1-std::abs(px-px_projected));
        w[2] *= (1-std::abs(py-py_projected));
        w[3] *= (1-std::abs(px-px_projected));
    }
...
// static int gradOutputOffset(lua_State *L){
//     THDoubleTensor *input = (THDoubleTensor*)luaT_checkudata(L, 1, 
// "torch.DoubleTensor");
//     THDoubleTensor *gradIm2col = (THDoubleTensor*)luaT_checkudata(L, 2, 
// "torch.DoubleTensor");
// }
